...
Module manifests can declare module-owned config defaults through the top-level `config` object. Normal module config remains plain local configuration. Built-in manifests may mark declared config keys with the built-in-only `configPrivate` field so displayable private metadata is encrypted at rest. Community modules cannot claim this trust classification or register colliding core protected keys such as `modem_password` or `admin_password`.
